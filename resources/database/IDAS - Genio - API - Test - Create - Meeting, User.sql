...
FROM (
	SELECT
		[MeetingTypeId]
		,[MeetingType] AS [Title]
		,[MeetingType] AS [Description]
		,'Teams Meeting' AS [Location]
		,1 AS [IsAllDayEvent]
		,[StartDate]
		,(RIGHT('00' + CAST(ROUND(RAND() * 8, 0) AS VARCHAR(2)), 2) + ':' + RIGHT('00' + CAST(ROUND(RAND() * 31, 0) AS VARCHAR(2)), 2)) AS [StartTime]
		,[EndDate]
		,(RIGHT('00' + CAST(ROUND(RAND() * 12, 0) AS VARCHAR(2)), 2) + ':' + RIGHT('00' + CAST(ROUND(RAND() * 31, 0) AS VARCHAR(2)), 2)) AS [EndTime]
