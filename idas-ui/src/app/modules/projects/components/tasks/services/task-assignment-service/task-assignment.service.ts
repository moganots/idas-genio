...
export class TaskAssignmentService extends DataService {
  ...
  mapValues(taskAssignment: TaskAssignment) {
    taskAssignment.Assignee = this.users.find(
      (user) => user._id === taskAssignment.AssigneeId
    );
    taskAssignment.createdBy = this.users.find(
      (user) => user._id === taskAssignment.CreatedBy
    );
    taskAssignment.modifiedBy = this.users.find(
      (user) => user._id === taskAssignment.ModifiedBy
    );
