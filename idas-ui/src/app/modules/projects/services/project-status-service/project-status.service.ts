...
export class ProjectStatusService extends DataService {
  ...
  mapValues(projectStatus: ProjectStatus) {
    projectStatus.Status = this.lookupValues.find(
      (lookupValue) => lookupValue._id === projectStatus.StatusId
    );
    projectStatus.createdBy = this.users.find(
      (user) => user._id === projectStatus.CreatedBy
    );
    projectStatus.modifiedBy = this.users.find(
      (user) => user._id === projectStatus.ModifiedBy
    );
