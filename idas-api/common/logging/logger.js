...
const setCaller = (caller) => {
  defaultCaller = caller || defaultCaller;
};
...
const log = (caller, logType, methodName, message, detailMessage = null) => {
  ...
  consoleMessage = `[${time}] [${logType.toLocaleUpperCase()}] /${caller}${
...
const onHttpRequestStarted = (caller, request) => {
  ...
  const message = [
    packet.absoluteUrlWithHttpMethod,
    `Http request initiated and processing`,
...
const onHttpRequestCompleted = (
  ...
  if (error) {
    return onHttpError(caller, request, response, {
      error: error,
      data: data,
      message: error.message || message,
    });
  } else {
    return onHttpSuccess(caller, request, response, {
      data: data,
      message: message,
    });
  }
  ...
  return Array.isArray(message)
    ? message
        ...
        .map(
          (msg) =>
            `${
              msg.type ? [msg.type, msg.message].join(`:`) : msg.message || msg
            }`
        )
        .join(`\r\n`)
...
const logToFile = (message) => {
  ...
  fs.appendFile(logFileFullName, message + "\n", function (err) {
    if (err) {
      error(__filename, `logFileFullName:fs.appendFile`, err.message || err, err);
    }
  });
};
...
const dataCount = (data) => {
  return hasData(data) && Array.isArray(data)
    ? data.length
    : hasData(data)
    ? 1
    : 0;
};
const hasError = (logType) => {
  return ![`debug`, `info`].includes(
    (logType || ``).toLocaleLowerCase().trim()
  );
};
